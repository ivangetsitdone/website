...
The preview key logs in as the dedicated `preview-deploy` system user and is restricted to the root-owned `/usr/local/sbin/website-preview-deploy` forced command; it cannot open an interactive shell or choose another command. Production deploys install that command and the reviewed public key in the AuthorizedKeysFile paths reported by the live SSH configuration, only after production is healthy. The candidate image is built and tested on an isolated GitHub runner, capped at 600 MB, exported with a SHA-256 checksum, and streamed to the forced command. The host verifies the checksum and exact `origin/preview` SHA, loads the image, and runs the trusted `/srv/website/compose.preview.yaml`; candidate Dockerfiles never execute on the production droplet.
