...
The separate preview key is installed as an additional `root` authorized key restricted to the root-owned `/usr/local/sbin/website-preview-deploy` forced command; it cannot open an interactive shell or choose another command. Production deploys install that command and authorized-key entry from the reviewed public key in `deploy/preview_deploy.pub`, only after production is healthy. The candidate image is built and tested on an isolated GitHub runner, capped at 600 MB, exported with a SHA-256 checksum, and streamed to the forced command. The host verifies the checksum and exact `origin/preview` SHA, loads the image, and runs the trusted `/srv/website/compose.preview.yaml`; candidate Dockerfiles never execute on the production droplet.
